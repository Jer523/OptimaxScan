...
/* ✅ BUG FIX 2b: 补上 margin-bottom 后的分号，border 才能生效 */
.stFileUploader {
...
def process_and_compress_to_letter(pil_img):
    ...
    # ✅ BUG FIX 1b: 确保传入的图像是 RGB，防止 RGBA/P 模式在 paste 时出错
    if pil_img.mode != 'RGB':
...
def process_scan_layered_from_mem(pil_img, is_small):
    ...
    if is_small:
        ...
        final = cv2.fastNlMeansDenoising(final, None, h=10, templateWindowSize=7, searchWindowSize=15)
        ...
        res = cv2.adaptiveThreshold(enhanced, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 51, 20)
...
} /* ✅ 补上这个闭合括号，sidebar-card 才不会被嵌套进去 */
...
with st.sidebar:
    st.markdown(f'''
        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin-bottom: -65px; padding-top: 20px; position: relative; z-index: 99;">
            <div style="width: 97px; height: 97px; border-radius: 50%; background: #F0F4F8; display: flex; align-items: center; justify-content: center; box-shadow: 6px 6px 12px #d1d9e6, -6px -6px 12px #ffffff;">
                <div style="width: 87px; height: 87px; border-radius: 50%; background: #F0F4F8; display: flex; align-items: center; justify-content: center; box-shadow: inset 4px 4px 8px #d1d9e6, inset -4px -4px 8px #ffffff; border: 1px solid rgba(255,255,255,0.9);">
    ...
    <div class="sidebar-card">
        <div style="text-align: center; font-size: 1.1em; font-weight: 600; margin-bottom: 1px;">支持格式 | FORMATS</div>
        <div style="text-align: center; font-size: 0.95em; color: #A0AEC0; font-weight: 400; line-height: 1.6; letter-spacing: 1.2px; margin-bottom: 10px;">PDF, JPG, PNG, HEIC</div>
        <div style="text-align: center; font-size: 0.95em; color: #A0AEC0; font-weight: 400; line-height: 1.4; margin-bottom: 1px;">支持多种格式批量上传</div>
        <div style="text-align: center; font-size: 0.94em; color: #A0AEC0; font-weight: 400; line-height: 1.6; letter-spacing: 0.7px">Mixed-format uploads</div>
    </div>
    ''', unsafe_allow_html=True)
    ...
    st.markdown('''
    <div class="sidebar-card">
        <div style="text-align: center; font-size: 1.1em; color: #64B8FF; font-weight: 600; margin-bottom: 7px">上传步骤 | TUTORIAL</div>
        <div style="text-align: left; font-size: 0.9em; color: #A0AEC0; font-weight: 400; line-height: 1.6; letter-spacing: 0.25px; margin-left: 14px">1. 上传素材 | Upload Files<br></div>
        <div style="text-align: left; font-size: 0.9em; color: #A0AEC0; font-weight: 400; line-height: 1.6; letter-spacing: -0.05px; margin-left: 14px">2. 开始优化 | Start Refining</div>
        <div style="text-align: left; font-size: 0.7em; color: #A0AEC0; font-weight: 400; line-height: 1.6; letter-spacing: 0.7px; margin-left: 28px">
            分析: 智能修复光线/清晰度<br>Auto-Fix Light and Clarity<br>
            压缩: 体积优化，画质无损<br>Lossless Size Optimization<br>
            排版: 统一 Letter Size 布局<br>Standard 8.5 × 11 Layout
        </div>
        <div style="text-align: left; font-size: 0.9em; color: #A0AEC0; font-weight: 400; line-height: 1.6; margin-left: 14px">3. 保存黑白 | Export B&W</div>
    </div>
    ''', unsafe_allow_html=True)
        ...
        for file_idx, file in enumerate(uploaded_files):
            ...
            if file.name.lower().endswith('.pdf'):
                try:
                    ...
                    for page in pages:
                        # ✅ BUG FIX 1a: PDF 页面强制转 RGB，防止 RGBA/其他模式导致 OpenCV crash
                        if page.mode != 'RGB':
                            page = page.convert('RGB')
                        # ✅ BUG FIX 1c: 限制尺寸，防止大 PDF 内存溢出
                        page.thumbnail((3000, 3000), Image.Resampling.BILINEAR)
                        temp_images.append(page)
                except Exception as e:
                    st.warning(f"PDF 转换失败: {file.name} — {e}")
                    continue
            else:
